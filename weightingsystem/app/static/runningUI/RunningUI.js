...
function getData(){
	...
	$.ajax({
		...
		data:{'eqp': dataSend},
		success:function(data){
			for (var i = data.id[data.id.length - 1]; i <= data.id[0]; i++) {
				if (data.fault_level[i - 1] == 2) {
					var addstr = '<tr class="danger" id="'+ i +'"><td>'+ i +'</td><td>'+ data.fault_time[i - 1] +'</td><td>'+ data.recover_time[i - 1] +'</td><td>'+ data.period_second[i - 1] +'</td><td style="color: red;">'+ data.fault_reason[i - 1] +'</td>'
					...
					var addstr = '<tr class="warning" id="'+ i +'"><td>'+ i +'</td><td>'+ data.fault_time[i - 1] +'</td><td>'+ data.recover_time[i - 1] +'</td><td>'+ data.period_second[i - 1] +'</td><td style="color: orange;">'+ data.fault_reason[i - 1] +'</td>'
					if (data.fault_state[i - 1] == '未修复'){
						addstr = addstr + '<td style="color: red;">' + data.fault_state[i - 1] + '<strong></strong></td></tr>'
					}
					else {
